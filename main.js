...
var resolveFetch4 = (customFetch2) => {
  if (customFetch2)
    return (...args) => customFetch2(...args);
  return (...args) => fetch(...args);
};
...
function makeHeaders(raw) {
  ...
  return {
    get(name) {
      var _a;
      return (_a = normalized[name.toLowerCase()]) != null ? _a : null;
    },
    has(name) {
    ...
    keys() {
      return Object.keys(normalized)[Symbol.iterator]();
    },
    values() {
      return Object.values(normalized)[Symbol.iterator]();
    },
    [Symbol.iterator]() {
      return Object.entries(normalized)[Symbol.iterator]();
    },
...
var customFetch = async (url, options = {}) => {
  ...
  const result = {
    ...
    clone: () => {
      return { ...result, bodyUsed: false, headers: makeHeaders(response.headers) };
    }
  };
  return result;
};
...
var SupabaseSyncSettingTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    noteBox.createEl("br");
    noteBox.createEl("br");
    noteBox.createEl("strong", { text: "\u{1F4C4} Sync scope: " });
    noteBox.createSpan({
      text: "Markdown (.md) files of any size and other vault files (images, PDFs, audio, canvas files, etc.) under 5 MB are synced."
    });
    new import_obsidian2.Setting(containerEl).setName("Supabase URL").setDesc("Your Supabase project URL").addText(
      (text) => text.setPlaceholder("https://xxxxx.supabase.co").setValue(this.plugin.settings.supabaseUrl).onChange((value) => {
        this.plugin.settings.supabaseUrl = value.trim();
        this.debouncedSave();
      })
    );
    new import_obsidian2.Setting(containerEl).setName("Service role key").setDesc("Your Supabase service_role key (keep secret)").addText((text) => {
      text.setPlaceholder("eyJhbGciOiJIUzI1NiIs...").setValue(this.plugin.settings.serviceRoleKey).onChange((value) => {
    ...
    new import_obsidian2.Setting(containerEl).setName("Auto-sync interval (minutes)").setDesc("Automatically sync on an interval. Set to 0 to disable.").addText(
      (text) => text.setPlaceholder("0").setValue(String(this.plugin.settings.autoSyncInterval)).onChange(async (value) => {
        const parsed = parseInt(value, 10);
        this.plugin.settings.autoSyncInterval = isNaN(parsed) ? 0 : Math.max(0, parsed);
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian2.Setting(containerEl).setName("Sync on Wi-Fi only").setDesc(
      "Skip automatic sync when on a cellular connection (mobile only). Manual sync is always allowed."
    ).addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.syncOnWifiOnly).onChange(async (value) => {
        this.plugin.settings.syncOnWifiOnly = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian2.Setting(containerEl).setName("Device name").setDesc(
      "A human-readable label for this device, used in conflict file names. Device identity for sync is handled by an auto-generated UUID (" + (this.plugin.settings.deviceId || "not yet generated") + "), so this name does not need to be unique."
    ).addText(
      (text) => text.setPlaceholder("e.g. laptop-ubuntu").setValue(this.plugin.settings.deviceName).onChange(async (value) => {
        this.plugin.settings.deviceName = value.trim();
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian2.Setting(containerEl).setName("Manual sync").setDesc("Trigger a sync immediately").addButton(
...
// state.ts
var STATE_PATH = ".obsidian/supabase-sync-state.json";
async function loadSyncState(vault) {
  try {
...
function pruneStaleEntries(state, localPaths, remotePaths) {
  const pruned = {};
...
// sync-engine.ts
var BUCKET = "obsidian-vault";
var TABLE = "file_sync";
var NON_RETRYABLE_STATUS_CODES = /* @__PURE__ */ new Set([400, 401, 403, 404, 405, 409, 422]);
async function retryWithBackoff(fn, maxRetries = 2, baseDelayMs = 1e3) {
  let lastError;
...
function isNonRetryableError(err) {
  ...
  if (lowerMsg.startsWith("invalid jwt") || lowerMsg.startsWith("permission denied") || lowerMsg.startsWith("unauthorized")) {
    return true;
  }
  return false;
}
...
async function uploadFile(supabase, path, content, checksum, deviceId, deviceName) {
  await retryWithBackoff(async () => {
    const contentType = getContentType(path);
    const { error: storageErr } = await supabase.storage.from(BUCKET).upload(path, content, { upsert: true, contentType });
    if (storageErr) {
...
async function downloadRemoteContent(supabase, path) {
  return retryWithBackoff(async () => {
    var _a;
    const { data, error } = await supabase.storage.from(BUCKET).download(path);
    if (error || !data) {
      throw new Error(`Storage download failed for ${path}: ${(_a = error == null ? void 0 : error.message) != null ? _a : "no data"}`);
    }
    return await data.arrayBuffer();
  });
}
async function writeLocalFile(vault, path, content) {
...
async function downloadFile(supabase, vault, path) {
  const content = await downloadRemoteContent(supabase, path);
  await writeLocalFile(vault, path, content);
  return content;
}
async function deleteFileRemotely(supabase, path, deviceId, deviceName) {
  await retryWithBackoff(async () => {
...
async function deleteFileLocally(vault, path) {
  const file = vault.getAbstractFileByPath(path);
  if (file instanceof import_obsidian3.TFile) {
    await vault.delete(file);
  }
}
function getConflictPath(originalPath, deviceName, updatedAtStr) {
...
async function handleLocalRename(supabase, vault, newPath, oldPath, deviceId, deviceName) {
  if (newPath.startsWith(".obsidian/"))
    return;
  try {
    const file = vault.getAbstractFileByPath(newPath);
    if (!(file instanceof import_obsidian3.TFile))
      return;
    if (file.extension !== "md" && file.stat.size >= 5 * 1024 * 1024)
      return;
    ...
    new import_obsidian3.Notice(`Sync failed for rename: ${msg}`);
  }
}
async function handleLocalDelete(supabase, vault, path, deviceId, deviceName) {
  ...
  try {
    ...
    new import_obsidian3.Notice(`Sync failed for delete ${path}: ${msg}`);
  }
}
async function sync(supabase, vault, deviceId, deviceName, lastRemotePruneTime, onProgress) {
  const result = { uploaded: 0, downloaded: 0, deleted: 0, errors: [] };
  const localState = await loadSyncState(vault);
  const { data: remoteRows, error: fetchErr } = await supabase.from(TABLE).select("path, checksum, updated_at, size, deleted, updated_by");
  if (fetchErr) {
    throw new Error(`Failed to fetch remote metadata: ${fetchErr.message}`);
  }
  const remoteMap = /* @__PURE__ */ new Map();
  for (const row of remoteRows != null ? remoteRows : []) {
    remoteMap.set(row.path, row);
  }
  const localFiles = vault.getFiles().filter((f) => !f.path.startsWith(".obsidian/") && (f.extension === "md" || f.stat.size < 5 * 1024 * 1024));
  ...
  await batchProcess(pathList, 5, async (path) => {
    ...
    try {
      ...
      const remoteDeleted = (remote == null ? void 0 : remote.deleted) === true;
      if (existsLocally && !existsRemotely) {
        if (lastKnown == null ? void 0 : lastKnown.deleted) {
        }
        const content = await vault.readBinary(file);
        const checksum = await computeChecksum(content);
        await uploadFile(supabase, path, content, checksum, deviceId, deviceName);
        localState[path] = { checksum, synced_at: (/* @__PURE__ */ new Date()).toISOString() };
        result.uploaded++;
        ...
        if (remoteDeleted) {
          ...
          const lastKnownChecksum = lastKnown == null ? void 0 : lastKnown.checksum;
          if (localChecksum === remoteChecksum) {
            ...
            if (localTime >= remoteTime) {
              ...
              new import_obsidian3.Notice(`\u26A0\uFE0F Conflict bootstrap on ${path}: remote version saved as ${conflictPath.split("/").pop()}`, 6e3);
              await uploadFile(supabase, path, content, localChecksum, deviceId, deviceName);
              localState[path] = { checksum: localChecksum, synced_at: (/* @__PURE__ */ new Date()).toISOString() };
              result.uploaded++;
            } else {
              ...
              new import_obsidian3.Notice(`\u26A0\uFE0F Conflict bootstrap on ${path}: local version saved as ${conflictPath.split("/").pop()}`, 6e3);
              const downloaded = await downloadFile(supabase, vault, path);
              const dlChecksum = await computeChecksum(downloaded);
              localState[path] = { checksum: dlChecksum, synced_at: (/* @__PURE__ */ new Date()).toISOString() };
              result.downloaded++;
            }
            ...
            const conflictPath = getConflictPath(path, remoteDeviceName, remote.updated_at);
            await writeLocalFile(vault, conflictPath, remoteContent);
            new import_obsidian3.Notice(`\u26A0\uFE0F Conflict on ${path}: remote version saved as ${conflictPath.split("/").pop()}`, 6e3);
            await uploadFile(supabase, path, content, localChecksum, deviceId, deviceName);
            localState[path] = { checksum: localChecksum, synced_at: (/* @__PURE__ */ new Date()).toISOString() };
            result.uploaded++;
          }
  ...
  if (shouldPrune) {
    ...
    const { error: pruneErr } = await supabase.from(TABLE).delete().eq("deleted", true).lt("deleted_at", thirtyDaysAgo.toISOString());
    if (pruneErr) {
...
async function handleRealtimeChange(supabase, vault, payload) {
  var _a, _b, _c, _d, _e, _f;
  const { eventType } = payload;
  if (eventType === "DELETE") {
    const path2 = (_a = payload.old) == null ? void 0 : _a.path;
    if (!path2) {
      console.warn(
        "[supabase-sync] Realtime DELETE event received with empty payload.old. This usually means REPLICA IDENTITY FULL is not set on the file_sync table. Run: ALTER TABLE file_sync REPLICA IDENTITY FULL;"
      );
      return;
    }
    if (!path2.endsWith(".md") || path2.startsWith(".obsidian/"))
      return;
    try {
      await deleteFileLocally(vault, path2);
      const localState = await loadSyncState(vault);
      if (localState[path2]) {
        localState[path2].deleted = true;
      ...
      new import_obsidian3.Notice(`Sync failed for delete ${path2}: ${msg}`);
    }
    return;
  }
  const path = (_b = payload.new) == null ? void 0 : _b.path;
  const remoteChecksum = (_c = payload.new) == null ? void 0 : _c.checksum;
  const remoteDeleted = ((_d = payload.new) == null ? void 0 : _d.deleted) === true;
  const remoteUpdatedBy = ((_e = payload.new) == null ? void 0 : _e.updated_by) || "remote";
  const remoteDeviceName = remoteUpdatedBy.includes(":") ? remoteUpdatedBy.split(":")[1] : remoteUpdatedBy;
  const remoteUpdatedAt = ((_f = payload.new) == null ? void 0 : _f.updated_at) || (/* @__PURE__ */ new Date()).toISOString();
  if (!path || !path.endsWith(".md") || path.startsWith(".obsidian/"))
    return;
  if (remoteDeleted) {
    try {
      await deleteFileLocally(vault, path);
  ...
  try {
    ...
    const existsLocally = localFile instanceof import_obsidian3.TFile;
    if (!existsLocally) {
      const downloaded = await downloadFile(supabase, vault, path);
      const dlChecksum = await computeChecksum(downloaded);
      localState[path] = {
    ...
    if (!lastKnownChecksum) {
      ...
      await writeLocalFile(vault, conflictPath, remoteContent);
      new import_obsidian3.Notice(`\u26A0\uFE0F Conflict on ${path}: remote saved as ${conflictPath.split("/").pop()}`, 6e3);
      localState[path] = {
        checksum: localChecksum,
        synced_at: (/* @__PURE__ */ new Date()).toISOString()
      };
    ...
    new import_obsidian3.Notice(`Sync failed for ${path}: ${msg}`);
  }
}

// main.ts
var _SupabaseSyncPlugin = class _SupabaseSyncPlugin extends import_obsidian4.Plugin {
  constructor() {
    super(...arguments);
  ...
  // ── Status Bar ────────────────────────────
  updateStatusBar(status, tooltip) {
    ...
    (0, import_obsidian4.setIcon)(this.statusBarItemEl, iconName);
  ...
  // ── Settings ──────────────────────────────
  generateDefaultDeviceName(isMobile) {
  ...
  async loadSettings() {
    ...
    if (!this.settings.deviceName) {
      this.settings.deviceName = this.generateDefaultDeviceName(import_obsidian4.Platform.isMobile);
      needsSave = true;
    }
   ...
   */
  isNetworkSuitable(isManual) {
    if (isManual) {
      if (typeof navigator !== "undefined" && !navigator.onLine) {
        new import_obsidian4.Notice("You are offline. Sync skipped.");
        return false;
      }
      return true;
    }
    ...
    if (this.settings.syncOnWifiOnly) {
      const conn = navigator == null ? void 0 : navigator.connection;
  ...
  async runSync(isManual = false) {
    if (!this.supabase) {
      new import_obsidian4.Notice("Supabase Sync: Please configure your Supabase URL and key in settings.");
      return;
    }
  ...
  async executeSyncLocked(isManual) {
    var _a, _b;
    if (!this.supabase)
      return;
    if (this.isSyncing) {
...
\u26A0\uFE0F ${result.errors.length} error(s):`;
        ...
        for (const e of shown) {
          const filePath = ((_a = e.split(":")[0]) == null ? void 0 : _a.trim()) || e;
          msg += `
  \u2022 ${filePath}`;
        }
  ...
  + ${result.errors.length - 3} more`;
      ...
      if (result.errors.length > 0 || isManual) {
        new import_obsidian4.Notice(msg, result.errors.length > 0 ? 8e3 : 4e3);
      }
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      ...
      if (isManual)
        new import_obsidian4.Notice(`Sync failed: ${msg}`);
    } finally {
      this.isSyncing = false;
      if (this.statusBarItemEl && ((_b = this.statusBarItemEl.getAttribute("aria-label")) == null ? void 0 : _b.includes("Syncing"))) {
        this.updateStatusBar("idle");
      }
    }
  }
  showSyncStatus() {
    const time = this.lastSyncTime ? new Date(this.lastSyncTime).toLocaleString() : "Never";
    const syncableFiles = this.app.vault.getFiles().filter(
...
Auto-sync: ${this.settings.autoSyncInterval > 0 ? `every ${this.settings.autoSyncInterval} min` : "disabled"}
Wi-Fi only: ${this.settings.syncOnWifiOnly ? "yes" : "no"}`
    ...
    this.registerEvent(
      this.app.vault.on("modify", (file) => {
        ...
        this.modifyDebounceTimer = setTimeout(() => {
          this.modifyDebounceTimer = null;
          this.runSync();
        }, _SupabaseSyncPlugin.MODIFY_DEBOUNCE_MS);
      })
    );
  }
  // ── Realtime ─────────────────────────────
  setupRealtimeSubscription() {
    if (!this.supabase)
      return;
    this.realtimeChannel = this.supabase.channel("vault-sync").on(
      "postgres_changes",
      {
        event: "*",
        schema: "public",
      ...
      async (payload) => {
        ...
        if (!this.supabase)
          return;
        const updatedBy = (_c = (_a = payload.new) == null ? void 0 : _a.updated_by) != null ? _c : (_b = payload.old) == null ? void 0 : _b.updated_by;
        if (updatedBy) {
          ...
          try {
            await handleRealtimeChange(
              ...
              this.app.vault,
              {
                ...
                new: (_a2 = payload.new) != null ? _a2 : {},
                old: (_b2 = payload.old) != null ? _b2 : {}
              }
            );
          } catch (err) {
            const msg = err instanceof Error ? err.message : String(err);
  ...
  teardownRealtimeSubscription() {
    ...
    if (this.realtimeChannel) {
      (_a = this.supabase) == null ? void 0 : _a.removeChannel(this.realtimeChannel);
      this.realtimeChannel = null;
    }
  }
  // ── Interval ─────────────────────────────
...
_SupabaseSyncPlugin.MODIFY_DEBOUNCE_MS = 5e3;
var SupabaseSyncPlugin = _SupabaseSyncPlugin;
